...
class ReportApi
{
     ...
     * @return \sendx\model\RestReportData|\sendx\model\ErrorResponse|\sendx\model\ErrorResponse|\sendx\model\ErrorResponse
     */
    public function getCampaignReport($identifier, string $contentType = self::contentTypes['getCampaignReport'][0])
    {
        list($response) = $this->getCampaignReportWithHttpInfo($identifier, $contentType);
        return $response;
    }
     ...
     * @throws \sendx\ApiException on non-2xx response or if the response body is not in the expected format
     * @throws \InvalidArgumentException
     * @return array of \sendx\model\RestReportData|\sendx\model\ErrorResponse|\sendx\model\ErrorResponse|\sendx\model\ErrorResponse, HTTP status code, HTTP response headers (array of strings)
     */
    public function getCampaignReportWithHttpInfo($identifier, string $contentType = self::contentTypes['getCampaignReport'][0])
    {
        $request = $this->getCampaignReportRequest($identifier, $contentType);
     ...
     */
    public function getCampaignReportAsync($identifier, string $contentType = self::contentTypes['getCampaignReport'][0])
    {
        return $this->getCampaignReportAsyncWithHttpInfo($identifier, $contentType)
            ->then(
                function ($response) {
                    return $response[0];
                }
     ...
     * @return \GuzzleHttp\Promise\PromiseInterface
     */
    public function getCampaignReportAsyncWithHttpInfo($identifier, string $contentType = self::contentTypes['getCampaignReport'][0])
    {
        $returnType = '\sendx\model\RestReportData';
        $request = $this->getCampaignReportRequest($identifier, $contentType);
     ...
     */
    public function getCampaignReportRequest($identifier, string $contentType = self::contentTypes['getCampaignReport'][0])
    {

        // verify the required parameter 'identifier' is set
        if ($identifier === null || (is_array($identifier) && count($identifier) === 0)) {
            throw new \InvalidArgumentException(
                'Missing the required parameter $identifier when calling getCampaignReport'
            );
        }
        if (!preg_match("/^[a-z_]+_[a-zA-Z0-9]{22}$/", $identifier)) {
            throw new \InvalidArgumentException("invalid value for \"identifier\" when calling ReportApi.getCampaignReport, must conform to the pattern /^[a-z_]+_[a-zA-Z0-9]{22}$/.");
        }
        

        $resourcePath = '/report/campaign/{identifier}';
        $formParams = [];
        $queryParams = [];
        $headerParams = [];
        $httpBody = '';
